...
alignment_quast$Contig_number <- str_split_fixed(alignment_quast$Contig, "_", 6)[,2] # Extracting the contig number
...
for(component in unique(reference_component$Component))
{
  ...
  {
    completeness_info <- data.frame(species = 'Enterococcus faecium',
  ...
  else
  {
    ...
    completeness_info <- data.frame(species = 'Enterococcus faecium',
                                    ...
                                    variation = max_variation,
                                    component = component,
                                    total_contigs = total_component_contigs,
                                    total_bp = total_component_bp,
...
for(component in unique(purity_components$Pair_contig_component))
{
  ...
  record_results <- data.frame( species = 'Enterococcus faecium',
                                classifier = 'mlplasmids', 
                                factor = 1.0,
                                number_iterations = number_iterations,
                                variation = max_variation,
