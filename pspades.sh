...
while getopts ":i:n:s:t:x:r:h" opt; do
 case $opt in
   h)
   echo "Welcome to the user guide to run the evaluation of plasmidSPAdes:"
...
echo -e "\n"
echo -e "!!!!!!Welcome to the script to evaluate plasmidspades!!!!!!\n"
...
then
  ...
  echo -e "You have given a reference genome to evaluate the results of plasmidspades:" $reference
...
snakemake --use-conda  -s pspades.smk pspades/"$name"_completeness.tab
